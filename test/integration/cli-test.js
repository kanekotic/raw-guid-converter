const coffee = require('coffee'),
    path = require('path'),
    pathToExecutable = path.join(__dirname, '../../bin/raw-to-guid.js')
...
describe('cli', () => {
    ...
    [
        ...
        it(`should convert ${guid[1]} string into ${guid[0]}`, async (done) => {

            coffee.spawn('node', [pathToExecutable, 'fromRaw', guid[0]])
    ...
    [
        ...
        it(`should convert miltiple ${guid[0][0]} ${guid[0][1]} string into ${guid[0][0]} ${guid[0][1]}`, async (done) => {

            coffee.spawn('node', [pathToExecutable, 'fromRaw', guid[0][0], guid[0][1]])
                .expect('stdout', guid[1])
                .expect('code', 0)
                .end(done)
        })
    ...
    [
        ...
        it(`should convert ${guid[1]} string into ${guid[0]}`, async (done) => {
            coffee.spawn('node', [pathToExecutable, 'fromString', guid[1]])
    ...
    [
        ...
        it(`should convert miltiple ${guid[1][0]} ${guid[1][1]} string into ${guid[0]}`, async (done) => {
            coffee.spawn('node', [pathToExecutable, 'fromString', guid[1][0], guid[1][1]])
                .expect('stdout', guid[0])
                .expect('code', 0)
                .end(done)
        })
